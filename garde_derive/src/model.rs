...
pub enum RawRuleKind {
    ...
    CharCount(Range<Either<usize, Expr>>),
    GraphemeCount(Range<Either<usize, Expr>>),
    Range(Range<Expr>),
    Contains(Expr),
    Prefix(Expr),
    Suffix(Expr),
...
pub enum ValidateRule {
    ...
    CharCount(ValidateRange<Either<usize, Expr>>),
    GraphemeCount(ValidateRange<Either<usize, Expr>>),
...
impl ValidateRule {
    pub fn name(&self) -> &'static str {
        match self {
            ...
            ValidateRule::CharCount { .. } => "char_count",
            ValidateRule::GraphemeCount { .. } => "grapheme_count",
